...
import type { RFQData, RFQFormStep } from "../types/rfq";
// import { createRFQ } from "../api/rfqApi" // Hapus baris ini
import { rfqService } from "../services/rfqService"; // Tambahkan baris ini
...
const RFQForm: React.FC<RFQFormProps> = ({
  ...
  const formSteps: RFQFormStep[] = [
    ...
    },
    {
      ...
      validation: (value) =>
        !value
          ? language === "id"
            ? "Spesifikasi ukuran wajib diisi"
            : "Size specifications are required"
          : null,
    },
    {
      step: 6,
  ...
  const validateCurrentStep = (): boolean => {
    ...
    if (step.required && !value) {
      setFormErrors({
        [step.field]: step.validation?.(value) || "This field is required",
      });
      return false;
    }
    ...
    if (value && step.validation) {
      ...
      if (error) {
        setFormErrors({ [step.field]: error });
        return false;
      }
    }
  ...
  // Koreksi fungsi ini untuk menggunakan rfqService.submitRFQ
  const handleRFQSubmit = async (dataFromForm: RFQFormData) => {
    // Menggunakan nama parameter berbeda untuk menghindari bayangan
    setIsSubmitting(true);
    setSubmissionError(null);
    setSubmissionSuccess(false);

    try {
      // Menggunakan rfqService untuk submit RFQ
      // Perhatikan bahwa rfqService.submitRFQ mungkin membutuhkan RFQData lengkap,
      // bukan hanya RFQFormData, tergantung implementasinya.
      // Anda mungkin perlu menyesuaikan pemetaan data di sini.
      const fullRfqData: RFQData = {
        userName: formData.userName || "", // Pastikan ada fallback atau validasi
        ...
        designFileUrls: uploadedFiles.map((file) => URL.createObjectURL(file)), // Ini mungkin perlu disesuaikan untuk URL sebenarnya setelah upload ke storage
      ...
      const submittedRfq = await rfqService.submitRFQ(fullRfqData); // Panggil rfqService.submitRFQ

      // Jika Anda juga perlu mengunggah file desain ke storage (seperti Supabase Storage),
      // Anda harus memanggil rfqService.uploadDesignFiles terlebih dahulu
      // dan kemudian menyimpan URL yang dihasilkan ke fullRfqData.designFileUrls
      // sebelum memanggil rfqService.submitRFQ.
      // Contoh:
      // const uploadedUrls = await rfqService.uploadDesignFiles(uploadedFiles);
      // fullRfqData.designFileUrls = uploadedUrls;
      // const submittedRfq = await rfqService.submitRFQ(fullRfqData);

      // Track RFQ submission
      await analyticsService.trackRFQSubmission(fullRfqData, user?.id);
      setSubmissionSuccess(true);
      reset();
      onSubmit(fullRfqData); // Panggil onSubmit dari props jika diperlukan
    } catch (error: any) {
      console.error("RFQ submission error:", error);
      setSubmissionError("Failed to submit RFQ. Please try again.");
    } finally {
  ...
  const renderStepContent = () => {
    const step = currentStepData;
    const value = formData[step.field];
    const error = formErrors[step.field];
        ...
        return (
          <div className="space-y-4">
            <div className="flex items-center mb-4">
              ...
              {step.field === "userEmail" && (
                <Mail className="h-6 w-6 text-green-500 mr-2" />
              )}
              {step.field === "projectName" && (
                <FileText className="h-6 w-6 text-green-500 mr-2" />
              )}
              {step.field === "sizeSpecifications" && (
                <Ruler className="h-6 w-6 text-green-500 mr-2" />
              )}
            ...
            <input
              ...
              placeholder={
                step.field === "userName"
                  ...
                  : step.field === "sizeSpecifications"
                  ? language === "id"
                    ? "Contoh: 10cm x 15cm, A4, Custom"
                    : "Example: 10cm x 15cm, A4, Custom"
                  : ""
              }
            />
        ...
        return (
          <div className="space-y-4">
            ...
            <input
              ...
              value={value || ""}
              onChange={(e) =>
                handleInputChange(step.field, Number.parseInt(e.target.value))
              }
              className={`w-full p-4 border rounded-lg text-lg ${
                error
                  ? "border-red-500"
        ...
        return (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "id"
                ? "Opsional - Kapan Anda membutuhkan proyek ini selesai?"
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {language === "id"
                    ? "JPG, PNG, PDF, AI, EPS, PSD (Max 10MB per file)"
                    : "JPG, PNG, PDF, AI, EPS, PSD (Max 10MB per file)"}
                </p>
              </label>
            </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <textarea
              ...
              placeholder={
                language === "id"
                  ? "Tambahkan informasi khusus atau permintaan lainnya..."
                  : "Add any special requirements or additional information..."
              }
            />
          </div>
        );
      ...
      >
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 p-6 text-white">
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm mb-2">
              <span>
                {language === "id" ? "Langkah" : "Step"} {currentStep + 1}{" "}
                {language === "id" ? "dari" : "of"} {formSteps.length}
              </span>
              <span>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-6 flex justify-between">
          ...
          {currentStep < formSteps.length - 1 ? (
            ...
            <button
              onClick={handleSubmit(handleRFQSubmit)}
