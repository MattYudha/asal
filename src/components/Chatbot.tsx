...
import { MessageCircle, X, Globe, Send, Paperclip, Printer, CheckCheck, Check, AlertCircle, Upload } from "lucide-react"
...
const ChatbotAvatar: React.FC<ChatbotAvatarProps> = ({ isBotMessage, photoURL }) => {
  return (
    <motion.div
      initial={{ scale: 0.8, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      className={`flex-shrink-0 w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br ${
        isBotMessage ? "from-green-500 to-green-800" : "from-gray-500 to-gray-700"
      }`}
    >
      {isBotMessage ? (
        <img
          src={photoURL || "https://example.com/bot_avatar.png"}
          alt="Bot Avatar"
          className="h-full w-full object-cover"
        />
      ) : (
        <div className="h-full w-full flex items-center justify-center text-white text-xs font-bold">You</div>
      )}
    </motion.div>
  )
}
      ...
      <form onSubmit={onSend} className="flex items-center space-x-2">
        <div className="flex-1 relative">
          <input
            ...
            className={`w-full py-2 px-3 pr-12 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 text-sm ${
              isFocused ? "border-green-300" : "border-gray-300 dark:border-gray-600"
        ...
        <motion.button
          ...
          className={`bg-gradient-to-br from-green-500 to-green-800 hover:from-green-600 hover:to-green-900 text-white p-2 rounded-lg transition-colors flex items-center justify-center ${
            isDisabled || (!value.trim() && !isImageUploading) || isImageUploading
...
const ChatbotMessage: React.FC<ChatbotMessageProps> = ({ message, isLastMessage, sessionId }) => {
    ...
    >
      {!isUserMessage && (
        <div className="mr-2 mt-1">
          <ChatbotAvatar isBotMessage={true} photoURL={message.photoURL} />
        </div>
      )}
      <div className={`max-w-[75%] flex flex-col ${isUserMessage ? "items-end" : "items-start"}`}>
        <motion.div
          ...
          {message.hasImage && message.imageUrl && (
            <div className="mb-2">
              <img
                ...
                loading="lazy"
              />
              {message.imageName && <p className="text-xs mt-1 opacity-75">{message.imageName}</p>}
            </div>
          )}
        ...
        <div className="flex items-center mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          {isUserMessage && !isLastMessage && <Check className="h-3 w-3 text-gray-400" />}
        </div>
      </div>
      {isUserMessage && (
        <div className="ml-2 mt-1">
          <ChatbotAvatar isBotMessage={false} photoURL={message.photoURL} />
        </div>
      )}
    </motion.div>
  )
...
const ChatbotSuggestions: React.FC<ChatbotSuggestionsProps> = ({ suggestions, onSelect }) => {
...
const Chatbot: React.FC = () => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        messages.push({
          ...
          sender: doc.data().uid === auth.currentUser?.uid ? "user" : "bot",
          uid: doc.data().uid,
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (message.trim()) {
      ...
      await analyticsService.trackChatbotInteraction("user", message, user?.id)
      ...
      setTimeout(async () => {
        const response = `AI Response: ${message}` // Echoing the user message for demonstration
        await addDoc(collection(db, "messages"), {
          text: response,
          createdAt: serverTimestamp(),
          uid: "bot", // Use a specific UID for the bot
          displayName: "Chatbot",
          photoURL: "https://example.com/bot_avatar.png", // Replace with a bot avatar URL
        })

        // Track AI response
        await analyticsService.trackChatbotInteraction("bot", response, user?.id)
  ...
  return (
    <>
      <div className="fixed bottom-4 right-4 z-50">
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              <ChatbotHeader title="Emran Chatbot - PT Emran Ghanim Asahi" onClose={toggleChat} />
              <div
              ...
              >
                ...
                <AnimatePresence>{isTyping && <ChatbotTypingIndicator />}</AnimatePresence>
                <AnimatePresence>{error && <ChatbotError message={error} onRetry={handleRetry} />}</AnimatePresence>
                <ChatbotSuggestions suggestions={suggestions} onSelect={handleSuggestionSelect} />
                <div ref={dummy} />
              </div>
              <ChatbotInput
                value={message}
                ...
                placeholder={
                  isImageUploading
                    ? t.imageAnalyzing
                    : t.typeMessage || (language === "id" ? "Ketik pesan Anda..." : "Type your message...")
                }
              />
            </motion.div>
          ) : (
      ...
      {/* RFQ Form Modal */}
      <AnimatePresence>
        {showRfqForm && <RFQForm onSubmit={handleRFQSubmit} onCancel={() => setShowRfqForm(false)} />}
      </AnimatePresence>
    </>
  )
}
